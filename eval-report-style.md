...
## Script generation — always write a script

- **Never hand-write the HTML report.** Always write a Python script that
  generates the report programmatically from `compare_experiments` output
  and `fetch_traces` data. The script should:
  1. Run `compare_experiments` and parse the structured output (use `--json`
     or the Python API if available; fall back to parsing text output).
  2. For every case, extract the trace URLs from the comparison output —
     `compare_experiments` already prints `trace1:` and `trace2:` lines per
     case. Parse those instead of hardcoding URLs.
  3. Optionally call `fetch_traces` for the top N regressed cases to get
     span counts, token counts, and tool-call sequences for root-cause
     analysis.
  4. Generate the HTML from a template, injecting the parsed data.
- Save the script alongside the report (e.g. `generate_report.py`) so the
  report can be regenerated by re-running the script with different tags.
- The script is the source of truth; the HTML is the output. If a URL or
  score is wrong, fix the script and regenerate — don't edit the HTML by
  hand.
